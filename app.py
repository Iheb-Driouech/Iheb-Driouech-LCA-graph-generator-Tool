...
def main():
    st.title("Analyse des données Excel - Application Streamlit")
    ...
    contributions_colors = None

    if uploaded_file is not None:
        st.write("Fichier bien reçu ! Lecture en cours...")
        ...
        if initial_table is not None and combined_table is not None:
            ...
            st.dataframe(initial_table)

            st.subheader("Tableau Combiné")
            st.dataframe(combined_table)

            st.subheader("Tableau Total Impact")
            st.dataframe(total_impact_table)

            percentage_table = generate_percentage_table(initial_table, total_impact_table)
            if percentage_table is not None:
                st.subheader("Tableau en pourcentage")
                st.dataframe(percentage_table)

            if st.button("Afficher le graphique de comparaison des scénarios"):
                st.write("Génération du graphique en cours...")
                fig_comparison = plot_comparison_bar_chart(total_impact_table)
                if fig_comparison is not None:
                    st.pyplot(fig_comparison)

            if st.button("Afficher les contributions relatives par scénario"):
                st.write("Construction des tables par scénario...")
                scenario_tables = generate_tables_by_scenario(initial_table, scenario_names_cleaned)

                st.write("Génération des graphiques de contribution (vertical) ...")
                fig_relative_vertical = plot_relative_contribution_by_scenario(scenario_tables)
                if fig_relative_vertical is not None:
                    st.pyplot(fig_relative_vertical)

                st.write("Génération des graphiques de contribution (horizontal) ...")
                fig_relative_horizontal = plot_relative_contribution_by_scenario_horizontal(scenario_tables)
                if fig_relative_horizontal is not None:
                    st.pyplot(fig_relative_horizontal)

            if percentage_table is not None and st.button("Afficher les graphiques empilés par catégorie"):
                st.write("Génération en cours...")
                fig_stacked = plot_stacked_bar_by_category(percentage_table, total_impact_table)
                if fig_stacked is not None:
                    st.pyplot(fig_stacked)

            if percentage_table is not None and st.button("Afficher le graphique combiné avec hachures"):
                st.write("Génération en cours...")
                fig_combined = plot_combined_graph_with_scenario_hatches(percentage_table, total_impact_table, show_totals=True)
